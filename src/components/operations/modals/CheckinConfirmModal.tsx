...
export const CheckinConfirmModal = () => {
  ...
  return (
    <>
      <AnimatePresence>
        {rejectingRequestId && (
          <div key="reject-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Overlay */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 shadow-2xl border border-slate-200 w-full max-w-md relative z-10 space-y-4"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center border border-rose-100">
                    <AlertTriangle className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-display font-bold text-base text-slate-850">Rechazar Solicitud</h3>
                    <p className="text-[11px] text-slate-400 font-mono mt-0.5">ID: {rejectingRequestId}</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-1">
                <label className="font-mono text-[9px] text-slate-400 font-bold uppercase block">
                  Motivo o Comentario de Rechazo
                </label>
                <textarea
                  value={rejectionNote}
                  onChange={(e) => setRejectionNote(e.target.value)}
                  placeholder="Por favor, ingresa la razón por la cual se rechaza la solicitud de ausencia..."
                  className="w-full text-xs font-semibold p-3 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 outline-none focus:bg-white focus:border-indigo-400 min-h-[100px] transition-all resize-none"
                  autoFocus
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                <button
                  ...
                  className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-lg transition-all cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className={`px-3.5 py-2 text-white text-xs font-bold rounded-lg transition-all shadow-sm flex items-center gap-1 cursor-pointer ${
                    rejectionNote.trim() 
